...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2017/06/11 13:42:57 by nbeny             #+#    #+#             */
/*   Updated: 2017/06/11 13:42:59 by nbeny            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "fdf.h"

void		ft_draw_line(t_3d p2, t_3d p3, t_env *e)
{
	t_color	c;
	t_line	l;
	...
	c.a = c.p0.x;
	c.b = c.p0.y;
	c.color = ft_color(*e, p2, p3);
	c = ft_color_min(e, c, p2, p3);
	if (c.pass > 10)
...
t_color		ft_color_min(t_env *e, t_color c, t_3d p2, t_3d p3)
{
	...
	ft_color_min_end(e, c);
	if (c.z_min == p3.z)
		c.real_color = c.color;
...
void		ft_printf_line(t_ctx ctx, t_env e)
{
	...
	while (ctx.tabp[j])
	{
		while (i != (stop - 1))
		{
			ft_draw_line(ctx.tabp[j][i], ctx.tabp[j][i + 1], &e);
			if (ctx.tabp[j + 1])
				ft_draw_line(ctx.tabp[j][i], ctx.tabp[j + 1][i], &e);
			i++;
